		...
		static void Main(string[] args)
		{
			...
			try
			{
				...
				if ( textResourcesList.Count > 0 )
				{
					...
					{
						//Check if we are only appending new ones
						//check if we are only appending
						if ( appenOnlyNewEntries && File.Exists(fileSaveName) )
						{
							// Open the output file.
							ResXResourceReader outReader = new ResXResourceReader( fileSaveName );

							foreach ( DictionaryEntry outdic in outReader )
							{
								if ( textResourcesList.ContainsKey( outdic.Key.ToString() ))
								{
									textResourcesList[outdic.Key.ToString()] = outdic.Value.ToString();
								}
							}
							outReader.Close();
